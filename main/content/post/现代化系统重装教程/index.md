...
为什么不应该再采用旧方式？现在几乎所有的电脑都已经是用了 UEFI 技术，旧技术对此毫无兼容性，这是原因之一。最重要的原因是，通用的重装系统方式往往采用所谓的“万能驱动”。事实上，一台电脑会出现的各种问题大多是由于驱动问题导致的，举例来说，无故的花屏、闪屏，显卡驱动出现问题的概率要远远大于操作系统出现问题的概率。如果盲目采用“万能驱动”，这些驱动并不适合自己的电脑，日后出现问题的可能性也大大提高。此外，厂商提供的驱动往往是最适合电脑、经过了特定优化的版本，这些版本的驱动是最安全、最稳定的。
...
本文操作方式完全参照个人习惯，做不到事无巨细，但会逐渐更新完善。这也是我重装系统多年以来经验的系统梳理。使用本教程重装，只适合个人的电脑，这也是较为常用的状态。本文章的前半部分是综述性的，在最后一部分会同步我个人的重装习惯，供参考。最后的内容是在综述的基础上，以“实事求是”的原则，在特定设备上给出的参考流程。

如果您是 ARM 平台的笔记本，方法会有一些不同，请参阅 [Windows on ARM 系统重装指南](https://eggroll.pages.dev/p/windows-on-arm-系统重装指南/)。
...
> 值得注意的是，内置驱动通常不需要更改。在某些特殊情况下，建议备份内置驱动（比如安装 Windows on ARM），但是除非出现问题如无法开机，不要导入内置驱动。**备份驱动仍然不是万能药。越新的驱动，其格式可能越奇怪，所以请读者者在备份驱动后，重装系统前，建议留存一份官方的直接驱动安装包，或者驱动官网提供的安装包以备不时之需。不过，就算读者不进行这一步，也是无妨的（进入系统后仍然可以操作）。**
...
> 下载结束后，务必校验，一旦镜像由于网络问题损坏，将产生无法预料的后果。建议使用 iso 格式的镜像，理由是后续可以方便地在创建虚拟机时复用，价值更高。为了 esd 的稍高的压缩率，将舍弃一些日后的便利性，请读者自行权衡。
...
1. 推荐：下载其他 PE ISO 镜像，再使用 PE 完成系统安装，推荐 `微PE`
2. 直接调用微软原版镜像完成安装（不推荐，若采用此方法，后文驱动注入步骤将完全失去意义，重装步骤也不再“现代化”）

在 PE 中，使用 CGI/Windows NT/Dism++ 安装镜像即可。

### 驱动注入

在 PE 中用 Dism++ 向刚安装好系统的分区注入驱动。步骤和备份驱动的方式高度一致，只需要点击“添加驱动”，并选择已经备份的驱动文件夹即可。完成此步后即可拔下 U 盘。

> 事实上，由于 PE 通常只是写入内存，如果您的镜像、驱动在其他硬盘分区，进入 PE 后，U 盘就已经是非必要的了。
...
1. 我的电脑有一块硬盘，应该分几个区？
    ...
    两个。鼓吹不分区与一定要分区的，都缺少实践逻辑。在阅读本文的读者，通常已经不再是电脑小白，假如要在“重装系统”这件事上节约时间，我最推荐的是分两个区。所有的数据和配置都留存在 D 盘，重装完成后用符号链接链接回 C 盘，即可在安装软件后无缝恢复数据。

2. 如何恢复软件的数据？

    我个人正在开发一款软件，待日后完成后，将同步到此文章。

---

## Windows 11 重装的标准流程【个人自用】

本部分是我个人参考的，可能有略微不符上文之处，初次参照请以上文为准。
...
- 配置组策略【值得注意的是，导入的组策略在刷新时间上完全随机，几乎不可用，只能手动设置，这样可以立即生效】
  ...
  - 小组件
    ...
    - 在锁屏界面上禁用小组件：已启用
  - 传递优化
    - 修改缓存驱动器：已启用：R:\DoSvcCache【需要先启用 imdisk】

- 查看 Edge 的版本号，酌情关闭 Edge 的自动升级，如果版本合适，可以在此处直接禁用 Edge 的所有更新任务计划和更新服务
  - 141 是一个很稳定的版本（目前没发现任何严重的问题），这个版本的 Edge 内置在 23H2/24H2 的 2025 年十月更新中（25H2 未测试版本，理论上也是 141），安装这个版本的系统时，在联网前就要关闭 Edge 的任务计划和更新服务
- 开启网络并静置数分钟
- 禁用 Windows 的索引服务并且关闭其自启动
- 启用 sudo，并选为内联
...
- 任务管理器应该始终置顶，永远保持最高的控制力
- 打开任务栏结束任务
- 对于台式机，登录选项中的“离开后重新登录”应该调整为“每次”，防止默认屏幕关闭后不会上锁

---

### 瑞昱网卡（蓝牙）的相关问题和解决方案（该问题主要出现在 RedmiBookPro）

- 右键托盘中的蓝牙图标，进入设置后，打开更多蓝牙设置，开启允许发现，有助于减少蓝牙的断连问题
- 设备管理器中关闭蓝牙的省电模式

### AMD 显卡驱动相关问题和解决方案（该问题主要出现在 UM780XTX）
...
虽说对与系统重装这件事，已经练到炉火纯青了，但是总归来说还是要培养平日里的实用习惯。从我的视角来分析，当我觉得系统变得“不干净”时，我总是会忍不住地去重装。不过说实话，这真的于我无益。

系统随着使用，一定会变得杂乱一些，但是，已经设定好的设置、软件的配置、开发环境、各种记录，都是一种宝贵的财富。一旦重装，必须花费大量的时间来恢复这些配置。

我能做的，是日常的清理、解耦，尽力减少自己重装系统的次数，把时间投入在真正有意义的事上。
