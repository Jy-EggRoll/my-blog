...
当您使用 Git 一段时间后，可能就会被 Git 的一些写法所困扰，这时本文仍然有用，可以用作一些参考，如后文的 [`.gitignore` 写法速查表](#gitignore-文件规则速查表)。

当您成为 Git 的深度用户后，就应该培养自己的提交规范了。详情请见 [Git 提交规范](https://eggroll.pages.dev/p/git-提交规范/)，这是我自己的提交习惯。
...
从我目前的使用经历来看，新手入门若使用命令行会大大增加入门的难度。我的建议是使用 VSCode，其内置了完善的 Git 图形化支持。
...
浅克隆默认只克隆主分支，而不会克隆其他分支，这会引入额外的工作量。好在，查阅 Git 官方文档，我找到了一个解决方案，即加上 `--no-single-branch` 参数。
...
倘若熟悉 VSCode，且偏好图形化操作，分支的切换可以在提交框的小按钮中完成。

使用分支久了之后，用户可能会面临一个问题：远程的分支已经删除，为什么本地还是可以看到 `origin/` 下的分支呢（使用 VSCode 的图形化界面时，这个问题会更明显）？这是因为，本地和远程的数据是相互独立的，当我们不指定 `--prune`（修剪）时，本地的数据不会默认删除远程已经失效的分支。
...
或者，在 VSCode 设置中打开 `git.pruneOnFetch`。

或者，使用 `git config --global fetch.prune true` 开启 Git 全局的修剪功能。

### 设置代理

由于网络原因，经常会出现克隆、拉取、推送失败的情况。这是由于 Git 默认不使用系统代理（通常情况下，浏览器基本都严格遵循系统代理，但其他软件不一定）。最简单的方法是在相关软件中开启“Tun”模式（或称虚拟网卡模式）。该模式的原理是，为电脑创建一张虚拟网卡，一切要访问互联网的请求都要经过这张虚拟网卡的处理。所以，对于不走系统代理的 Git，自然也可以被妥善处理。该方法的缺点是，判断一切流量，所以性能较差。纵使如此，这仍然是我最推荐的方法。
